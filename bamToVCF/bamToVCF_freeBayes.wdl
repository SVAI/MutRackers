...
task HaplotypeCaller {
  ...
  command {
    freebayes --fasta-reference ${ref_fasta} ${input_bam}
  }
  runtime {
    docker: "heuermh/freebayes"
    memory: "10 GB"
    cpu: "1"
    disks: "local-disk " + disk_size + " HDD"
    preemptible: preemptible_tries
  ...
  output {
    File output_vcf = "${vcf_basename}.vcf"
  }
}
...
task IndexVCF {
  ...
  output {
    File output_vcf_index = "${vcf_basename}.idx"
  }
}
...
workflow bamToVCF{
  ...
  call MergeVCFs {
    input:
      ...
      # TODO - 
      input_vcfs_indexes = IndexVCF.output_vcf_index,
